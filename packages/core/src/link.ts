 ...
 */
export function resolveTweetLink(tweet: Tweet, host: XitterHost): string {
 ...
 */
export function resolveUserProfile(tweet: Tweet, host: XitterHost): string {
  if (host.type === 'twitter') {
 ...
 * @returns {string} A URL that resolves into user profile
 */
export function resolvePhotoLink(tweet: Tweet, host: XitterHost, index: number): string {
  if (host.type === 'twitter') {
